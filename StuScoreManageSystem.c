...
STU *DeleteScore(STU *head, int *stuNum) {
    ...
    // 对于删除尾节点
    if(head != NULL && rowNum == *stuNum && head->next != NULL) {
    ...
    // 对于删除非头非尾节点
    if (head != NULL && rowNum < *stuNum && rowNum != 1) {
        // 遍历至对应行号的前驱节点
...
STU *SortbyScoreAscending(STU *head, int stuNum) {
    ...
    // 注意理解：我们增加一个节点，放在第一个节点的前面，主要是为了便于比较。因为第一个节点没有前驱，我们不能交换地址
    p1->next = head;
    // 让 head 指向 p1 节点，排序完成后，我们再把 p1 节点释放掉
    head = p1;
    for (endpt = NULL; endpt != head; endpt = p) {
        for (p = p1 = head; p1->next->next != endpt; p1 = p1->next) {
...
void Print(STU *head, int stuNum, int courseNum) {
    ...
    if (head != NULL) {
        ...
        printf("\t\t    **************************************************************************************\n");
        printf("\t\t    %2s%6s%12s%8s%8s%8s%8s%8s%8s%8s%9s\n",
